...
const printOutsidersContent = (groupNumber = 'الأولى', outsidersData = []) => {
  const tableHeaders = [
    'الرتبة',
    'المسمي الوظيفي',
    'الاسم',
          ...
          <tbody>
            ${outsidersData.map((row, index) => `
              <tr${index % 2 === 1 ? ' style="background-color: #fafafa;"' : ''}>
                ...
                <td>${row.reason}</td>
                <td>${row.date}</td>
                ${row.date === 'صح' && `<td>من ${row.exitDate} الي ${row.expectedReturn}</td>`}
...
export default printOutsidersContent;
