...
const printOutsiders = (squadData) => {
  const tableHeaders = [
    'الرتبة',
    'المسمي الوظيفي',
    'الاسم',
    ...
    <body>
      <div class="print-container">
        <div class="header">
          ...
          <h2>السرية ${squadData[0].squadName}</h2>
          <h3>كشف الخوارج</h3>
          <div class="date">
            <p>${currentDate}</p>
        ...
        <table>
          ...
          <tbody>
            ${squadData.map((row, index) => `
              <tr${index % 2 === 1 ? ' style="background-color: #fafafa;"' : ''}>
                <td>${row.rank}</td>
                <td>${row.jobTitle}</td>
                <td>${row.name}</td>
                <td>${row.reason || ''}</td>
                <td>${row.hasDate ? '<span style="color: green;">✔️</span>' : '<span style="color: red;">✖️</span>'}</td>
                ${row.hasDate && `<td>من ${row.dateFrom} الي ${row.dateTo}</td>`}
